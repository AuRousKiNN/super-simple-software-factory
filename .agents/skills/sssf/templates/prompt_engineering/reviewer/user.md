...
1. Establish the target from `work_item`; if absent, establish the spec: read `<context_handoff_dir>/plan.md` if it exists, else use `prompt`.
2. Read `previous_envelope.diff_path`, then inspect the code named by its deterministic `changed_files` index.
